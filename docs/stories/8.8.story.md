...
4. Crosstown side of bootstrap flow correctly passes `peerAddress` through the full chain: SPSP request → `negotiateAndOpenChannel()` → `channelClient.openChannel({ peerAddress })` — verified by test
5. Crosstown side passes `peerAddress` through `ConnectorChannelClient.openChannel()` — already in the interface (Story 7.3)
...
### Crosstown Side Already Correct
...
**AC4**: Crosstown side passes `peerAddress` through full chain — verified by test
