...
5. `@crosstown/bls` imports TOON codec from `@crosstown/core` (no local copy remains)
6. `@crosstown/relay` imports TOON codec from `@crosstown/core` (no local copy remains)
7. All existing BLS and relay tests pass with updated import paths (`pnpm -r test`)
8. TOON codec exports are available from `@crosstown/core` package index

## Tasks / Subtasks

- [x] Task 1: Move encoder to core (AC: #1, #8)
  - [x]Copy `packages/bls/src/toon/encoder.ts` to `packages/core/src/toon/encoder.ts`
  - [x]Change error base class from `BlsBaseError` to `CrosstownError` (import from `../errors.js`)
...
- [x] Task 2: Move decoder to core (AC: #2, #8)
  ...
  - [x]Change error base class from `BlsBaseError` to `CrosstownError`
...
- [x] Task 6: Update BLS imports (AC: #5, #7)
  ...
  - [x]Update `packages/bls/src/toon/index.ts` to re-export from `@crosstown/core` (or delete and update all BLS import paths to import from `@crosstown/core` directly)
  - [x]Add `@crosstown/core` as a dependency in `packages/bls/package.json` if not already present
...
- [x] Task 7: Update relay imports (AC: #6, #7)
  ...
  - [x]Update `packages/relay/src/toon/index.ts` to re-export from `@crosstown/core` -- must include `encodeEventToToonString` since internal relay files (ConnectionHandler.ts, NostrRelayServer.test.ts) import it from `../toon/index.js`
...
This is a code extraction and reorganization story. The TOON encoder/decoder already exist and work correctly in both `@crosstown/bls` and `@crosstown/relay` (they are near-identical copies). The task is to consolidate them into `@crosstown/core` and create a new shallow parser. No new encoding/decoding logic is invented.

### Why This Story Exists

The SDK (`@crosstown/sdk`) needs TOON codec access. If SDK depends on BLS or relay for the codec, it creates circular dependencies when `@crosstown/town` (built on SDK) also provides relay functionality. Moving the codec to `@crosstown/core` (the shared foundation package) breaks this cycle.
...
**Core error base class**: `CrosstownError` in `packages/core/src/errors.ts` -- this is what the moved error classes should extend.

### Error Class Migration

BLS uses `BlsBaseError` and relay uses `RelayError` as base classes for TOON errors. In core, both `ToonEncodeError` and `ToonError` must extend `CrosstownError`:
...
- `CrosstownError(message, code, cause?)` -- passes `cause` through to `Error` via `super(message, { cause })` using the standard `Error` cause option
- In the new core versions, pass `cause` as the third arg to `super()`. Do NOT set `this.cause` manually -- `CrosstownError` handles it via `Error`'s built-in cause mechanism.
...
- `packages/core/package.json`: `@toon-format/toon` is currently in `devDependencies` at `^1.1.0`. It MUST be moved to `dependencies` (not just verified). Without this, consuming packages that depend on `@crosstown/core` will get a runtime error when the TOON codec tries to import `@toon-format/toon`.
- `packages/bls/package.json`: `@crosstown/core` is already in `dependencies` at `workspace:*`. `@toon-format/toon` at `^1.0.0` is also a direct dependency -- it can be removed after migration since BLS will get it transitively through core, but this is optional cleanup.
- `packages/relay/package.json`: `@crosstown/core` is already in `dependencies` at `workspace:*`. `@toon-format/toon` at `^1.0.0` is also a direct dependency -- same optional cleanup as BLS.

### Re-export Strategy for BLS and Relay

Two valid approaches for updating BLS/relay:

**Option A (recommended): Thin re-export files.** Keep `packages/bls/src/toon/index.ts` and `packages/relay/src/toon/index.ts` as thin re-export files pointing to `@crosstown/core`:
...
**Option B: Direct imports.** Delete the toon directories entirely and update every import site to use `@crosstown/core` directly. More churn but cleaner long-term.

Choose whichever approach results in fewer changes and lower regression risk.

### BLS/Relay Error Compatibility

After migration, `ToonError` and `ToonEncodeError` will be `instanceof CrosstownError` (not `BlsBaseError` or `RelayError`).
...
| T-1.0-07 | Re-export from `@crosstown/core` index.ts works (import validation)    | Unit        | E1-R01 | P1       |
...
- **Error classes extend CrosstownError** -- not `Error`, `BlsBaseError`, or `RelayError`
...
- [Source: _bmad-output/planning-artifacts/architecture.md#Modified Package: @crosstown/core]
...
- [Source: packages/core/src/errors.ts] -- CrosstownError base class
...
- **Task 1 (Move encoder to core)**: Verified `packages/core/src/toon/encoder.ts` contains `encodeEventToToon` and `encodeEventToToonString`, error class extends `CrosstownError` with proper cause handling, `@toon-format/toon` is in `dependencies` (not `devDependencies`).
- **Task 2 (Move decoder to core)**: Verified `packages/core/src/toon/decoder.ts` contains `decodeEventFromToon`, `ToonError` extends `CrosstownError`, includes `validateNostrEvent()` and `isValidHex()` helpers.
...
- **Task 6 (Update BLS imports)**: Verified BLS toon/index.ts is a thin re-export from `@crosstown/core`. Old encoder.ts, decoder.ts, toon.test.ts deleted. BLS `package.json` has `"test": "vitest run"`. Created `packages/bls/vitest.config.ts` to fix test discovery when running from package directory. BLS tests: 10 files, 233 tests passed.
- **Task 7 (Update relay imports)**: Verified relay toon/index.ts is a thin re-export from `@crosstown/core` (includes `encodeEventToToonString`). Old encoder.ts, decoder.ts, toon.test.ts deleted. Relay `package.json` has `"test": "vitest run"`. Created `packages/relay/vitest.config.ts` to fix test discovery. Relay tests: 9 files, 216 tests passed.
...
- `packages/core/src/toon/encoder.ts` -- created (moved from BLS/relay, error class changed to CrosstownError)
- `packages/core/src/toon/decoder.ts` -- created (moved from BLS/relay, error class changed to CrosstownError)
...
- `packages/bls/src/toon/index.ts` -- modified (thin re-export from @crosstown/core)
...
- `packages/relay/src/toon/index.ts` -- modified (thin re-export from @crosstown/core, includes encodeEventToToonString)
