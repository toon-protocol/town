...
research_topic: 'Marlin Protocol integration into TOON'
research_goals: 'Identify valuable use cases and innovations for integrating Marlin into the TOON protocol'
user_name: 'Jonathan'
date: '2026-03-05'
...
- Technology Stack - Oyster runtime, TEE platforms, smart contracts, TOON stack compatibility
...
_TOON Alignment: VERY HIGH -- Both protocols implement pay-per-use models. TOON uses ILP/SPSP for payment; x402 uses HTTP 402 + USDC. Complementary rather than competing approaches._
...
_Emerging Pattern: Marlin is positioning as chain-agnostic confidential compute layer with payment built in -- directly parallel to TOON's chain-agnostic payment layer with relay built in._
...
### API Design Patterns: TOON-Marlin Touchpoints

The integration between TOON and Marlin connects at four distinct API boundaries:

**1. Relay-to-Enclave (WebSocket ↔ vsock proxy):**
TOON's `NostrRelayServer` accepts WebSocket connections on port 7100. Marlin's dual-proxy architecture transparently bridges vsock isolation, supporting HTTP and WebSocket without application modification. A TOON relay running inside an Oyster CVM would expose its WebSocket endpoint through Marlin's inbound proxy -- clients connect identically, but the relay runs in a hardware-isolated TEE with attestable code integrity.
...
TOON's `createHttpConnectorAdmin()` communicates with the ILP connector via REST (`POST /admin/peers`, `DELETE /admin/peers/{id}`, `POST /admin/channels`). This internal API could remain host-side or be enclave-resident depending on trust model requirements.

**4. Smart Contract Bridge (Solidity ↔ Solidity):**
Both ecosystems use Solidity contracts on EVM chains. TOON's `TokenNetwork` contracts (payment channels) and Marlin's relay contracts (job submission, attestation verification) could share the same chain. Marlin's `relayJob()` + `oysterResultCall()` callback pattern could trigger TOON operations verified by TEE attestation.
...
| Dimension | TOON (ILP/SPSP) | Marlin (x402) |
|-----------|---------------------|---------------|
...
A bridge could enable TOON relays to accept x402 payments alongside ILP, or Marlin services to settle through ILP payment channels. The shared secp256k1 key infrastructure makes cross-protocol identity mapping straightforward.
...
TOON's relay returns events in TOON (binary) format over WebSocket. Marlin's proxy architecture handles WebSocket transparently with TLS termination. The TOON codec's compact binary format is advantageous in enclave environments where memory is fixed and I/O traverses vsock -- fewer bytes means less proxy overhead.

**Scallop RA-TLS for Relay-to-Relay Trust:**
TOON peers currently discover each other via kind:10032 events and establish trust through SPSP handshakes. Marlin's Scallop protocol adds hardware-backed authentication: two TOON relays running in Oyster CVMs could establish RA-TLS channels, cryptographically proving they run authorized code before peering. This eliminates the need for pre-shared secrets or manual peer approval.
...
Both TOON and Marlin use Docker as their deployment primitive. TOON's `docker-compose-read-only-git.yml` defines a multi-service stack (relay + BLS + connector + Anvil). Marlin's Oyster CVM accepts Docker images directly -- the existing TOON Docker image could be deployed to Oyster with minimal modification, primarily adding attestation server configuration and proxy endpoint mapping.
...
**TOON Service Decomposition for Oyster:**
...
If an Oyster CVM becomes unavailable (enclave failure, operator downtime), TOON's existing architecture can fall back to non-TEE operation. Oyster's monitoring protocol penalizes nodes for downtime and reassigns tasks, but TOON should implement its own degradation path: detect attestation staleness, warn peers, and optionally continue in reduced-trust mode.
...
TOON's Nostr relay already functions as a pub/sub bus for peer discovery (kind:10032). Extending this with TEE attestation events creates a decentralized attestation registry:
...
Oyster Serverless supports subscription-based periodic execution. These could monitor TOON relay events and trigger compute:
...
TOON uses Nostr keypairs (secp256k1/Schnorr) for identity. Marlin's Nautilus KMS provides persistent keys that survive enclave restarts. Integration pattern: derive Nostr identity keys inside the enclave using KMS seeds, so the relay's identity is bound to its TEE -- if the code changes, the key changes, making impersonation cryptographically impossible.
...
The fundamental architectural pattern for Marlin-TOON integration is the **TEE-backed decentralized relay network** -- a hybrid of TOON's existing publish/subscribe relay mesh with Marlin's confidential compute layer providing hardware-enforced trust guarantees.

**Current TOON Architecture:**
```
...
The Ethereum ecosystem faces censorship concerns with MEV relays (e.g., bloXroute censoring OFAC-sanctioned transactions). TOON+Marlin sidesteps this by making the relay logic hardware-attested. If a relay operator deploys a censoring variant, the PCR measurement changes, and peers can detect and avoid it.
...
The blockchain coprocessor pattern -- where expensive computation is performed off-chain with results posted back on-chain with a proof -- maps directly to TOON's architecture:

| Computation | Current (TOON) | With Marlin Coprocessor |
|-------------|---------------------|------------------------|
...
TOON's EVM payment channels (state channels) and Marlin's TEE execution create a powerful synergy. State channels handle micropayment scalability (off-chain balance updates, on-chain settlement), while TEEs guarantee the payment logic itself is tamper-proof. Together, they provide:
...
Integrating Marlin's operator marketplace with TOON's payment model creates a three-token economic structure:
...
2. **TOON relay fees**: AGENT tokens from pay-per-byte event publishing (existing)
...
**Challenge**: TOON uses SQLite (`events.db`) for event storage. Enclave restarts would lose state.
...
**Oyster Marketplace as TOON Deployment Platform:**

Instead of operators running `deploy-genesis-node.sh` on their own infrastructure, they could deploy TOON nodes via the Oyster marketplace:

1. **Build**: Package TOON Docker image with Nix reproducible builds
...
- **Permissionless scaling**: Anyone with POND stake can become a TOON relay operator
...
The integration of Marlin into TOON should follow a **strangler fig pattern** -- progressively wrapping existing components in TEE enclaves while maintaining backward compatibility at each phase.

**Phase 1: TEE-Optional Deployment (Weeks 1-4)**
- Package existing TOON Docker image for Oyster CVM deployment
...
```bash
# Existing
pnpm build && docker build -t toon-node .
...
1. **Reproducible builds**: Docker builds are not inherently deterministic. Nix integration is required to ensure PCR values are reproducible across different build environments. TOON's existing `pnpm build` must produce identical output regardless of build machine.
...
TOON's existing `/health` endpoint on the BLS server should be extended:
...
- Existing TOON team handles relay/BLS/connector work
...
```
Month 1: Foundation
├── Package TOON Docker image for Oyster CVM
...
| Uptime SLA | 99.5% | Oyster monitoring + TOON health checks |
...
## Research Synthesis: Verifiable Relay Infrastructure — Marlin Protocol Integration into TOON

### Executive Summary

The integration of Marlin Protocol's Oyster confidential computing platform into TOON represents a transformative opportunity to create the first **verifiably neutral, pay-per-use relay network** -- a system where relay operators provably cannot censor, reorder, or modify events because the relay code runs in hardware-attested Trusted Execution Environments.

This research identifies Marlin-TOON as a natural architectural fit. Both protocols operate at complementary layers of the decentralized stack: TOON provides the publish/subscribe relay mesh with ILP payment channels, while Marlin provides the confidential compute substrate with TEE attestation and an operator marketplace. Their shared technology foundations (TypeScript, Solidity, Docker, secp256k1 cryptography) minimize integration friction.

The most significant discovery is the **x402 payment protocol bridge** -- Marlin's HTTP-native payment standard using USDC that directly complements TOON's ILP/SPSP model. Together, they enable dual payment paths: ILP channels for high-volume streaming micropayments between established peers, and x402 for one-off API access without requiring channel setup. This expands TOON's addressable market from ILP-capable clients to any HTTP client with a crypto wallet.
...
- **Minimal code changes**: TOON's existing Docker image can deploy to Oyster CVM with primarily configuration additions (attestation server, proxy endpoints)
...
- **Data Sources**: Marlin official documentation, GitHub repositories, blog posts, protocol specifications; TOON codebase analysis; industry research (CoinTelegraph, Messari, Archetype Fund); academic papers on confidential computing
...
**Original Goal**: Identify valuable use cases and innovations for integrating Marlin into TOON
...
**Oyster CVM**: Dedicated confidential VMs running any Linux program with persistent state, no time limits, 100% dedicated resources. Ideal for long-running services like TOON relay nodes.
...
**Critical Implementation Insight**: Marlin's Docker-native deployment model means the primary work is **configuration, not code rewriting**. TOON's existing Docker image deploys to Oyster CVM with additions for attestation server endpoint, proxy port mapping, and KMS configuration. The relay, BLS, and bootstrap code run unmodified inside the enclave.
...
| Technology | TOON | Marlin | Compatibility |
|------------|-----------|--------|---------------|
...
For TOON's relay workload (WebSocket handling, TOON encoding, SQLite writes), the TEE overhead is negligible relative to network I/O latency.
...
2. **Build the x402 payment bridge** (Phase 3, ~4 weeks): Deploy Marlin's x402-gateway as reverse proxy. Accept USDC for HTTP API access alongside existing ILP channels. Opens TOON to any HTTP client with a wallet.
...
**Critical Risk**: Marlin network availability. **Mitigation**: TEE is additive, not required. TOON continues to function without Marlin. Self-hosted deployment remains supported throughout all phases.

### 10. Future Outlook and Innovation Opportunities

**Near-Term (6-12 months):**
- TEE-attested relay becomes a competitive differentiator for TOON
- x402 adoption grows (Coinbase-backed, 100M+ payments processed)
- Marlin's GPU TEE support (NVIDIA H100) enables AI inference inside TOON enclaves

**Medium-Term (1-2 years):**
- TOON could become a **decentralized infrastructure provider** -- relay-as-a-service deployed via Oyster marketplace
- Cross-chain attestation verification (Marlin already supports Arbitrum, Sui) enables multi-chain TOON settlement
...
| TOON codebase | Local code | Relay, BLS, connector, SDK architecture |
...
#### Appendix B: TOON Contract Addresses (Anvil)
...
Marlin Protocol provides a production-ready confidential computing infrastructure that aligns architecturally with TOON at every layer. The integration creates a unique position in the market: the first **verifiably neutral, pay-per-use social protocol relay** backed by hardware attestation. The x402 payment bridge discovery expands the economic model beyond ILP-only to include HTTP-native USDC payments, dramatically lowering the barrier to entry for new users and AI agents.

### Strategic Impact Assessment

This integration positions TOON as infrastructure for **trust-minimized social communication** -- a category that doesn't yet exist but is enabled by the convergence of TEEs, payment channels, and social protocols. The phased adoption strategy ensures zero risk to existing functionality while progressively adding verifiable trust guarantees.

### Next Steps

1. Package TOON Docker image for Oyster CVM and deploy a test relay (1-2 days of effort)
...
_This comprehensive technical research document serves as an authoritative reference on Marlin Protocol integration into TOON and provides strategic technical insights for implementation planning and decision-making._
