...
describe('Pricing Validator', () => {
  it.skip('[P0] underpaid event produces F04 rejection with required/received metadata', () => {
    // Arrange
  ...
  it.skip('[P0] self-write bypass accepts event from own pubkey regardless of amount', () => {
    // Arrange
  ...
  it.skip('[P0] default basePricePerByte is 10n when not specified', () => {
    // Arrange
    const validator = createPricingValidator({
      ownPubkey: 'ff'.repeat(32),
    });
  ...
  it.skip('[P1] overpaid event is accepted', () => {
    // Arrange
    const validator = createPricingValidator({
      basePricePerByte: 10n,
      ownPubkey: 'ff'.repeat(32),
  ...
  it.skip('[P1] exactly-priced event is accepted', () => {
    // Arrange
    const validator = createPricingValidator({
      basePricePerByte: 10n,
      ownPubkey: 'ff'.repeat(32),
