...
export async function runPush02(
  ...
  // Reused blobs (README.md, package.json, index.ts) are automatically
  // skipped by delta logic in uploadGitObject when their SHAs are in shaMap.
  const uploadOrder: { obj: ReturnType<typeof createGitBlob>; type: 'blob' | 'tree' | 'commit' }[] = [
    // Reused blobs — will be skipped by delta logic
  ...
  for (const { obj, type } of uploadOrder) {
    // Delta logic in uploadGitObject skips already-uploaded SHAs
    // Only sign claims for new objects
    if (shaMap[obj.sha]) {
      continue;
    }
