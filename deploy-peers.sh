...
# Check if toon-network exists
if ! docker network inspect toon-network &>/dev/null; then
    log_error "toon-network does not exist"
...
deploy_peer() {
    ...
    local ILP_ADDRESS="g.toon.peer${PEER_NUM}"
...
services:
  ...
  connector-peer${PEER_NUM}:
    ...
    labels:
      - "toon.group=peer-${PEER_NUM}"
      - "toon.role=connector"
      - "toon.tier=application"
    environment:
      ENVIRONMENT: development
      LOCAL_DELIVERY_URL: http://toon-peer${PEER_NUM}:${BLS_PORT}

      # Payment channels
      BASE_ENABLED: "true"
      BASE_RPC_URL: http://toon-anvil:8545
      BASE_CHAIN_ID: "31337"
  ...
  # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  # TOON Node for Peer #${PEER_NUM}
  # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  toon-peer${PEER_NUM}:
    image: toon:optimized
    container_name: toon-peer${PEER_NUM}
    labels:
      - "toon.group=peer-${PEER_NUM}"
      - "toon.role=relay"
      - "toon.tier=application"
...
EOF
    ...
    docker compose -p "toon-peer-${PEER_NUM}" -f "$COMPOSE_FILE" up -d forgejo-peer${PEER_NUM}
    ...
    while [ $elapsed -lt $timeout ]; do
        if docker compose -p "toon-peer-${PEER_NUM}" -f "$COMPOSE_FILE" ps forgejo-peer${PEER_NUM} | grep -q "healthy"; then
    ...
    local TOKEN_OUTPUT=$(docker exec --user git forgejo-peer${PEER_NUM} forgejo admin user generate-access-token \
        ...
        --token-name "TOON-BLS-NIP34-$(date +%s)" 2>&1)
    ...
    # For now, restart toon with token set via docker exec
    # (Better approach: use .env file per peer, but this works)

    # Step 6: Start remaining services
    log_info "Starting connector and toon services..."
    docker compose -p "toon-peer-${PEER_NUM}" -f "$COMPOSE_FILE" up -d

    # Step 7: Configure Forgejo token in running container
    sleep 5
    docker exec toon-peer${PEER_NUM} sh -c "echo 'FORGEJO_TOKEN=${FORGEJO_TOKEN}' >> /app/.env" 2>/dev/null || true
    docker compose -p "toon-peer-${PEER_NUM}" -f "$COMPOSE_FILE" restart toon-peer${PEER_NUM}
    ...
    log_info "Waiting for TOON node to be healthy..."
    elapsed=0
    while [ $elapsed -lt $timeout ]; do
        if curl -sf "http://localhost:${BLS_PORT}/health" &>/dev/null; then
            log_success "TOON node is healthy"
            break
        fi
        sleep 2
        elapsed=$((elapsed + 2))
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  View peer logs:
    ${CYAN}docker compose -p toon-peer-1 -f docker-compose-peer1.yml logs -f${NC}

  Stop all peers:
    ${CYAN}for i in ${DEPLOYED_PEERS[*]}; do docker compose -p toon-peer-\$i -f docker-compose-peer\$i.yml down; done${NC}

  Access Forgejo:
    ${CYAN}Peer 1: http://localhost:3014 (user: toon, pass: toon1)${NC}
